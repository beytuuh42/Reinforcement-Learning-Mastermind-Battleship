...
INPUT_SHAPE_FLAT = (ROWS*COLUMNS*2+sum([v for v in SHIPS.values()]))
...
class BattleshipEnvironment(py_environment.PyEnvironment):
    ...
    def _step(self, action):  
        ...
        ship_obs = np.array([not ship.is_alive(x, y) for ship in self._ships], dtype=np.int32)
        self._observation = np.hstack((self._misses.flatten(), self._hits.flatten(), ship_obs))
